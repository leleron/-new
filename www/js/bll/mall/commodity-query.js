                                ...
                                function(r) {
                                    if (r.status == "SUCCESS") {
                                        ...
                                        $.each(r.data,function(index){
                                            if(Number(r.data[index].sales_number) > 10000 ){
                                                var currentSalesNumber1 = parseInt(r.data[index].sales_number/10000);
                                                var currentSalesNumber2 = parseInt(r.data[index].sales_number%10000/1000);
                                                r.data[index].sales_number = currentSalesNumber1+"."+currentSalesNumber2+"万";
                                            }
                                        });
                                        /*商品列表的bind*/
    ...
    $("#commodityDetails1").on("panelload",
        function () {
            ...
            $("#go-to-chat").off("tap").on("tap",function(){
                if(loj.Credential){
                    var ref = cordova.InAppBrowser.open('http://feike2.1shen7.com/new/client.php?' +
                        'm=Mobile&arg=admin&tokenId='+loj.Credential, '_blank', 'location=no');
...
var getHotProduct = function(r) {
    ...
    $.each(remai, function(i) {
        ...
        svc.goodSalesNumber(goods_sn, function(r) {
            if (r.status == "SUCCESS") {
                remai.eq(i).find("span")[0].innerHTML = r.data.sales_number;
            } else {
                hlp.myalert(r.message);
            }
        });
