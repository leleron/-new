            ...
            if (r.status == "SUCCESS") {
                ...
                $("#actionPay").off("tap").on("tap", function () {
                    ...
                    if (payType == "appalipay") {
                        flycoAlipay(orderSn, price, payType);
                    } else if (payType == "appunionpay") {
                        flycoUnionPay(orderSn, price, payType);
                    } else if (payType == "appwxpay") {
                        flycoWxPay(orderSn, price, payType);
                    }
                });
                        ...
                        var getIntegralSuccessCallback=function(integral){
                            ...
                            $('#integral').off("input").on("input", function () {
                                ...
                                if(integral_now.trim().length==0){
                                    ...
                                    if(parseFloat(integral_now)>parseFloat(thisOrderCanUserIntegra)){
                                        $('#integral').val(thisOrderCanUserIntegra);
                                        updateAllPrice(sum_price,thisOrderCanUserIntegra,freight);
                                        return;
                                    }else{
                            ...
                            $("#submitOrderPay").off("tap").on("tap", function () {
                                ...
                                svc.addOrder(userId,addressId,goods,goodSuit,invType,companyName,integral,isCart,function(r){
...
//支付宝
var flycoAlipay = function(orderSn, price, payType) {
    svc.getPayPrepare(orderSn, price, payType, function(r) {
        if (r.status == "SUCCESS") {
            // 交易号
...
// 微信支付
var flycoWxPay = function(orderSn, price, payType) {
    svc.getPayPrepare(orderSn, price, payType, function(r) {
        if (r.status == "SUCCESS") {
            // 交易号
            var tradeNo = r.data.out_trade_no;
...
// 银联支付
var flycoUnionPay = function(orderSn, price, payType) {
    svc.getPayPrepare(orderSn, price, payType, function(r) {
        if (r.status == "SUCCESS") {
            // 交易号
            var tradeNo = r.data.out_trade_no;
