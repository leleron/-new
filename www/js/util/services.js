var ShowLoginMsg = function (bool) {
    var lmsg = "<a href='#login'><p align='center' class='topMessage'>您的用户凭证已过期，请重新登录！</p></a>";
    $(".topMessage").remove();
    if (bool == true){
...
Services.prototype = {
    ...
    addr_compareVersion: rootuser + "/find/currentVersion",
    addr_resetpwdbyemail: rootuser + "/user/email",
    ...
    addr_randomcode: rootuser + "/user/randomcode",
    ...
    addGeneralToken: function(data) {
        ...
        if (localStorage.FLX_USER == undefined || localStorage.FLX_USER.trim().length == 0 ||
            ...
            JSON.parse(localStorage.FLX_USER).CredentialStatus == "inactive") {
            data["token"] = "";
            data.user_id="";
        } else {
    ...
    //第三方登录
    thirdPartyLoginIn: function(loginType, uid, accessToken,nickName,callback) {
        var data = {
            "LOGINTYPE": loginType,
            "UID": uid,
    ...
    //版本比较
    getVersionInfo: function(currentVersion, appType, callback) {
        var url = this.psurl(this.addr_compareVersion, []);
        var data = {
            APPTYPE: appType,
    ...
    //邮箱重置密码
    resetPwdByEmail: function(email, callback) {
        var data = { EMAIL: email };
        this.processpost(this.addr_resetpwdbyemail, data, callback);
    },
    ...
    getRandomcode: function(callback) {
        var url = this.psurl(this.addr_randomcode);
        this.processget(url, callback)
    },
    ...
    //獲取套裝
    getSuit:function(type,callback){
        var data = {
            "type":type
        };
        this.processpost(this.mall_suitCommodity,this.addGeneralToken(data),callback);
    },
