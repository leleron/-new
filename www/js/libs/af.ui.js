...
(function ($) {
    ...
    AFUi.prototype = {
         ...
         * set the back button link
         ```
         $.afui.setBackButtonLink("about");
         ```
         * @param {string} text
         * @title $.afui.setBackButtonLink(title)
         */
        setBackButtonLink: function (url) {
            var obj = $(this.activeDiv).closest(".view").find("header .backButton"); //.html(text);
            obj.removeClass("backButton").addClass("customer_back").attr("href", url);
        },
        /**
         * set the back button link
         ```
         ...
         * @title $.afui.setBackButtonLink(title)
         */
        resumeBackButton: function () {
            var obj = $(this.activeDiv).closest(".view").find("header .customer_back");
            obj.remove();
        },
        /**
        ...
        blockUIwithMask: function (text) {
            var that = this;
            that.blockUI(0.5);
            that.showMask(text);
            that.blocked = true;
            that.timeoutblocks = that.timeoutblocks + 1;

            window.setTimeout(function(){
                if (that.blocked && that.timeoutblocks==1) {
                    that.unblockUI();
                    that.hideMask();
                }
                that.timeoutblocks = that.timeoutblocks - 1;
        ...
        loadContent: function (target, newView, back, transition, anchor) {
            ...
            // all transition will be reset to none.
            // transition = "none";

            if (target.length === 0 || target.indexOf("#") === -1) return;
            if (target.indexOf("#") !== -1) {
        ...
        showBackButton: function (view, isNewView) {
            ...
            if (items >= 2 && isNewView !== true) {
                ...
                hdr.prepend("<a class='backButton back'></a>");
            }
            else {
        ...
        runTransition: function (transition, previous, target, back, noTrans) {
            // reset all transition to none.
            transition = "none";

            console.log("BEFORE::inside run transition...history...");
            var hist = this.views["mainview"];
            for (var i = 0; i < hist.length; i++) {
                console.log("TARGET::::" + hist[i].target.id);
            }
            console.log("AFTER::inside run transition...history...");
            ...
            console.log("::::HIDE:" + $(hide).attr("id"));
            if (back)
                $(hide).trigger("backcomplete");
            console.log("::::SHOW:" + $(show).attr("id"));
        },
        ...
        updateViewHistory: function (view, div, transition, target) {
            ...
            // avoid edit mode.
            var hist = this.views[view.prop("id")];
            if (hist && hist.length > 2 && (hist[hist.length - 2].target.id == div.id)) {
                this.views[view.prop("id")].pop();
            } else {
        ...
        launch: function () {
            ...
            var maskDiv = $.create("div", {
                id: "afui_mask",
                className: "ui-loader",
                html: '<div class="spinner"> <div class="spinner-container container1"> <div class="circle1"></div> <div class="circle2"></div> <div class="circle3"></div> <div class="circle4"></div> </div> <div class="spinner-container container2"> <div class="circle1"></div> <div class="circle2"></div> <div class="circle3"></div> <div class="circle4"></div> </div> <div class="spinner-container container3"> <div class="circle1"></div> <div class="circle2"></div> <div class="circle3"></div> <div class="circle4"></div> </div> </div>'
            }).css({
                "z-index": 20000,
                display: "none"
            });
            ...
            $(document).on("click", "a", function (e) {
                // check authentication before loadcontent
                var tgt = e.target;
                var tagname = e.target.tagName;
                var dccode = $(tgt).attr("data-check");

                if (typeof (dccode) == "undefined")
                    dccode = "";

                // hlp.log("tagName:" + tagname + ",dc:" + dccode);
                if (!Authentication(tagname, dccode))
                    return false;
