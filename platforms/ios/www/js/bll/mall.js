$(function () {
    ...
    $("#pay,#submitOrder").on("panelload",function(){
        $(".showMallButton").hide();
    })
    ...
    $('#mallIndex').on('panelload',function(){
        $('#myPanel li a').on('tap',function(){
            if(loj.IsLogin){
                $.afui.loadContent(this.getAttribute('data-href'));
            }else{
...
function showMallPanel(_this){
    if(_this=='myPanel'){
        $('.down-list').slideUp();
        return ;
    }
    $('.down-list').slideToggle(500);
}
