...
@interface SYQRCodeViewController () <AVCaptureMetadataOutputObjectsDelegate,ZBarReaderDelegate,UIImagePickerControllerDelegate,QUMockDelegate>
{
}
...
-(void)QUMock:(QUMock *)mock entity:(QUEntity *)entity{
    ...
    if ([mock isKindOfClass:[checkBindedMock class]]) {
        ...
        if ([e.status isEqualToString:RESULT_SUCCESS]) {    //二维码未被绑定 去配网
            ...
            if ([se.type isEqualToString:@"7008"]) {
                ...
                _myAddParam.PRODUCTCODE = se.type;
                _myAddParam.MACADDRESS = se.idCode;
                _myAddParam.SN = se.idCode;
                _myAddParam.NICKNAME = se.idCode;
